...
\pkg{ggmcmc} is an \proglang{R} package for analyzing Markov chain Monte Carlo simulations from Bayesian inference.
  By using a well known example of hierarchical / multilevel modelling, the article reviews the potential uses and options of the package, ranging from classical convergence tests to caterpillar plots or posterior predictive checks.
}
...
The \code{ggmcmc()} wrapper that writes all plots into a pdf file is reviewed in Section~\ref{s:ggmcmc}.
...
\pkg{ggmcmc} is under development.
The latest version and the discussions about its future can be followed at \url{http://github.com/xfim/ggmcmc}.


\section[Why ggmcmc]{Why \pkg{ggmcmc}?}
\label{s:why}
\pkg{ggplot2}, based on the grammar of graphics~\cite{wilkinson:2005:grammar}, empowers \proglang{R} users by allowing them to flexibly create graphics~\cite{wickham:2009:ggplot2}.
...
There are currently two general-purpose packages for convergence analysis and diagnostics in \proglang{R}: \pkg{coda}~\cite{coda} and \pkg{boa}~\cite{boa}.
...
\pkg{ggplot2} is based on the idea that the input of any graphic is a data frame mapped to aesthetic attributes (colour, size) of geometric objects (points, lines).
...
The flexibility comes from the fact that it is very easy to extend basic graphics by including more aesthetic elements and geometric objects, and even faceting the figure to generate the same plot for different subsets of the dataset \cite[p.~3]{wickham:2009:ggplot2}.
...
Getting \code{ggplot} objects as the output of the plotting functions has a positive side effect: while the defaults in \pkg{ggmcmc} have been carefully chosen, the user later can tweak any graph by adding other geometric figures, layers of data, contextual information (titles, axis) or applying themes.

To sum up, the implementation is driven by the following steps:
\begin{enumerate}
  \item Convert any input into a data frame using \code{ggs()}, producing a tidy object~\cite{wickham:2014:tidy}.
...
This approach also allows \pkg{ggmcmc} to work well with other tools designed to work with data frames, such as \pkg{dplyr} and \pkg{tidyr} (which, in fact, are \pkg{ggmcmc} dependencies).
...
\label{s:get_radon}
In order to show show the potential of \pkg{ggmcmc} with real data and a well-known dataset, this section briefly presents \citeasnoun{gelman:hill:2006}'s example of radon and floor measurements for 919 houses from 85 counties in Minnesota.
The model is a varying intercepts and slopes with group-level predictors presented in Chapter 17.2
...
@
<<load_development_package, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE>>=
library("devtools")
...
\item \textbf{Chain} Number of the chain.
\item \textbf{Parameter} Name of the parameter.
\item \textbf{value} value sampled.
\end{itemize}
...
The resulting object is tidy~\cite{wickham:2014:tidy}.
...
\begin{description}
  \item[\proglang{JAGS}] which produces objects of class \code{mcmc.list} \cite{jags}.
  \item[\pkg{MCMCpack}] which produces objects of class \code{mcmc} \cite{mcmcpack}.
  \item[\pkg{rstan}] which produces objects of class \code{stanfit} \cite{stan}.
  \item[\proglang{Stan} running alone] when \proglang{Stan} runs from the command line it produces csv files \cite{stan}.
\end{description}

\section[Using ggmcmc()]{Using \code{ggmcmc()}}
\label{s:ggmcmc}
\code{ggmcmc()} is a wrapper to several plotting functions that allows to create very easily a report of the diagnostics in a single pdf file.
...
While autocorrelation is not \emph{per se} a signal of lack of convergence, it may indicate some misbehaviour in several chains or parameters, or indicate that a chain needs more time to converge.
...
The Potential Scale Reduction Factor ($\hat{R}$)~\cite[296--297]{gelman:bayesian:2003} relies on different chains for the same parameter, by comparing the between-chain variation with the within-chain variation.
...
By contrast the Geweke z-score diagnostic ~\cite{geweke:1992} focuses on the comparison of the first part of the chain with its last part.
...
This feature is very useful when plotting the varying slopes or intercepts of several groups in multilevel modelling against a continuous feature of such groups.
...
The separation plot \cite{greenhill:2011:separation} is obtained by \code{ggs\_separation()}.
...
First, extract the credible intervals (including the median) of the parameters that start with \code{alpha} or \code{beta}, and using the pipe operator, keep only the variables ``Parameter'' and ``median''.
<<>>=
...
I would like to thank code contributions and ideas from Zachary M. Jones (\url{http://www.zmjones.com/}, Julien Cornebise (\url{http://www.cornebise.com/julien/}), and ideas from Dieter Menne (\url{http://www.menne-biomed.de/}) and Bill Dixon.
