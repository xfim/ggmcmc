...
* **`rstan`** which produces objects of class `stanfit` [@stan].
* **`Stan` running alone** when `Stan` runs from the command line it produces csv files [@stan].



# Using ggmcmc()
`ggmcmc()` is a wrapper to several plotting functions that allows to create very easily a report of the diagnostics in a single pdf file.
...
# Acknowledgements
I would like to thank code contributions and ideas from [Zachary M. Jones][zachary], [Julien Cornebise][julien], [Max Joseph][maxjoseph], and ideas from [Dieter Menne][dieter] and Bill Dixon.
