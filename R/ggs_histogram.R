...
#' @param binwidth Number of bins in the histogram, defaults to 30.
#' @return A \code{ggplot} object.
#' @export
#' @examples
#' data(samples)
#' ggs_histogram(ggs(S, parallel=FALSE))
ggs_histogram <- function(D, family=NA, binwidth = 30) {
  # Manage subsetting a family of parameters
  if (!is.na(family)) {
    D <- get_family(D, family=family)
  }
  # Plot
  f <- ggplot(D, aes(x=value)) + geom_histogram(binwidth = binwidth) +
