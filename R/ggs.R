...
#'
#' This function manages MCMC samples from different sources (JAGS, MCMCpack, STAN -both via rstan and via csv files-) and converts them into a data frame. The resulting data frame has four columns (Iteration, Parameter, value, Chain) and six attributes (nChains, nParameters, nIterations, nBurnin, nThin and parallel). The ggs object returned is then used as the input of the ggs_* functions to actually plot the different convergence diagnostics.
#'
...
#' @export
#' @return D A data frame with the data arranged and ready to be used by the rest of the \code{ggmcmc} functions. The data frame has four columns, namely: Iteration, Parameter, value and Chain, and six attributes: nChains, nParameters, nIterations, nBurnin, nThin and parallel.
#' @examples
